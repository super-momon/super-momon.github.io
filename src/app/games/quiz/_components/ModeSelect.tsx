...
const SCORING = [
  { id: 'easy' as QuestionDifficulty, label: 'Easy', pts: '+1', color: 'text-green-500', activeBg: 'rgba(34,197,94,0.07)', activeBorder: 'rgba(34,197,94,0.4)', activeGlow: 'rgba(34,197,94,0.1)' },
  { id: 'medium' as QuestionDifficulty, label: 'Medium', pts: '+2', color: 'text-yellow-500', activeBg: 'rgba(234,179,8,0.07)', activeBorder: 'rgba(234,179,8,0.4)', activeGlow: 'rgba(234,179,8,0.1)' },
  { id: 'hard' as QuestionDifficulty, label: 'Hard', pts: '+3', color: 'text-orange-500', activeBg: 'rgba(249,115,22,0.07)', activeBorder: 'rgba(249,115,22,0.4)', activeGlow: 'rgba(249,115,22,0.1)' },
  { id: 'extra-hard' as QuestionDifficulty, label: 'X-Hard', pts: '+5', color: 'text-red-500', activeBg: 'rgba(239,68,68,0.07)', activeBorder: 'rgba(239,68,68,0.4)', activeGlow: 'rgba(239,68,68,0.1)' },
] as const;
...
export function ModeSelect({ onStart, onOpenLeaderboard }: Props) {
  ...
  function renderDifficultySignal(id: QuestionDifficulty, active: boolean) {
    const barsCount = id === 'easy' ? 1 : id === 'medium' ? 2 : id === 'hard' ? 3 : 4;
    return (
      <div className="flex gap-0.5 items-end h-2.5 mb-1.5 opacity-80">
        {[1, 2, 3, 4].map((i) => {
    ...
    return (
      <div className="flex flex-col mb-4 w-full">
        <div className="flex items-center gap-2">
          <span className="flex items-center justify-center w-5 h-5 rounded-full text-[10px] font-extrabold bg-accent/10 border border-accent/20 text-accent">
            {number}
          </span>
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 pt-36 pb-24 select-none">
      <motion.div
        variants={container}
        initial="hidden"
        animate="show"
      ...
      >
        ...
        <motion.div variants={item} className="w-full mb-8">
          ...
          >
            <svg className="w-3.5 h-3.5 text-slate-500 dark:text-slate-400 group-hover:text-foreground shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
            ...
            {MODES_LIST.map((m) => {
              ...
              return (
                <motion.button
                  ...
                  className={[
                    'group relative p-6 rounded-2xl border text-left cursor-pointer overflow-hidden transition-[border-color,background-color,box-shadow] duration-300 outline-hidden focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-background',
                    isSelected
                ...
                >
                  ...
                  <div className="font-bold text-base text-foreground mb-1 group-hover:text-foreground/90 transition-colors duration-200">
                    {m.label}
                  </div>
                  <div className={`text-[10px] font-semibold tracking-wide uppercase mb-3 ${m.accentClass}`}>
                    {m.tagline}
                  </div>
        ...
        <motion.div variants={item} className="w-full grid grid-cols-1 gap-5">
          {/* Categories card */}
          <div
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2 shrink-0 self-start mt-0.5">
                <button
                  ...
                  className="text-[10px] uppercase font-bold text-accent hover:text-accent-hover cursor-pointer transition-colors outline-hidden focus-visible:ring-1 focus-visible:ring-accent"
                >
                  All
                </button>
                ...
                <button
                  ...
                  className="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-350 cursor-pointer transition-colors outline-hidden focus-visible:ring-1 focus-visible:ring-slate-400"
                >
                  None
                </button>
              </div>
              ...
              {(Object.keys(CATEGORY_LABELS) as CategoryKey[]).map((cat) => {
                ...
                return (
                  <button
                    ...
                    className={[
                      'relative px-3 py-2 rounded-xl text-[11px] font-semibold cursor-pointer border text-center transition-all duration-200 hover:-translate-y-[1px] outline-hidden focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-2 focus-visible:ring-offset-background',
                      active
                        ? 'border-accent/40 text-foreground font-bold shadow-2xs'
                  ...
                  >
                    <span className="flex items-center justify-center gap-1.5 w-full">
                      {active && (
                        <svg className="w-3 h-3 text-accent shrink-0" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
          ...
          >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-2">
              {SCORING.map(({ id, label, pts, color, activeBg, activeBorder, activeGlow }) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      boxShadow: active ? `0 4px 15px ${activeGlow}` : undefined,
                    }}
                  >
                    ...
                    <div className="flex flex-col items-center pt-2.5">
                      ...
                      {renderDifficultySignal(id, active)}
        ...
        {/* CTA & Actions */}
        <motion.div variants={item} className="flex flex-col items-center gap-3.5 mt-2">
          <motion.button
            disabled={!canStart}
          ...
          >
            ...
            <span className="relative flex items-center justify-center gap-2">
              ...
              {canStart && (
                <svg className="w-4 h-4 transition-transform duration-250 group-hover/btn:translate-x-1" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
